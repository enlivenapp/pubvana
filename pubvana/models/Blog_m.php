...
class Blog_m extends CI_Model
{
	...
	public function inc_post_count($id = false)
	{
		if (!$id)
		{
			return;
		...
		$post_count = $this->db->select('post_count')->where('id', $id)->limit(1)->get($this->_table['posts'])->row();
		...
		if ($post_count)
		{
			...
			$this->db->where('id', $id)->update($this->_table['posts'], ['post_count' => $count]);
		}
	}
