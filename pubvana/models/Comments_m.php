...
class Comments_m extends CI_Model
{
		...
		if ($query->num_rows() > 0)
		{
			...
			foreach ($result as &$item)
			{
				...
				if ($item->user_id)
				{
					...
					$item->author = $this->pv_auth->get_db_display_name($item->user_id);
				}
			}
			// return tastiness
			return $result;
	...
	public function create_comment($id)
	{
		...
		if ( $this->pv_auth->logged_in() )
		{
			// yes
			$data = [
				'post_id' 		=> $id,
				'user_id' 		=> $this->pv_auth->get_user_id(),
		...
		if ($this->db->insert($this->_table['comments'], $data))
		{
			...
			$this->pvcore->send_email($this->config->item('admin_email'), $this->config->item('site_name') . ' ' . lang('email_new_comment_sbj'), lang('email_new_comment_msg') . $this->input->post('comment'));
			return true;
		}
		return false;
	}
